...
import { Mail, Car, Loader2, ArrowLeft, CheckCircle } from 'lucide-react';
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || t('forgotPassword.failed', 'Failed to send reset email. Please try again.');
      setError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-[400px] animate-fade relative z-10">
        ...
        <div className="bg-white rounded-2xl p-8 md:p-10 shadow-elevated border border-[#e8e6e1]/60">
          <div className="text-center mb-8">
            <h1 className="text-lg sm:text-xl font-bold text-[#1e293b] mb-1.5">{t('forgotPassword.title', 'Reset Password')}</h1>
            <p className="text-slate-500 font-normal text-xs sm:text-sm">{t('forgotPassword.subtitle', "Enter your email and we'll send you a reset link.")}</p>
          </div>
          ...
          {sent ? (
            <div className="text-center py-6">
              ...
              <h3 className="text-lg font-semibold text-[#1e293b] mb-2">{t('forgotPassword.sentTitle', 'Check your inbox')}</h3>
              <p className="text-slate-500 text-sm mb-6">{t('forgotPassword.sentDesc', "If an account exists with that email, you'll receive a password reset link.")}</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                >
                  {loading ? (
                    ...
                    <span>{t('forgotPassword.sendLink', 'Send Reset Link')}</span>
                  )}
                </button>
              </form>
