...
export default function RolePermissions() {
  ...
  useEffect(() => {
    api.get('/permissions/matrix')
      ...
      .catch(() => showToast('Unable to load role permissions'))
  ...
  const toggle = async (role: string, code: string, enabled: boolean) => {
    ...
    try {
      ...
      showToast('Role access updated');
    } catch {
      showToast('Unable to update role access');
    } finally {
      setSavingKey('');
    }
  };
  ...
  if (!matrix) {
    return <div className="p-6 text-sm text-slate-500">Role permissions are unavailable.</div>;
  }
