...
import { CheckCircle, Copy, Eye, FileText, Image, Layers, Plus, Save, Search, Star, Trash2, Upload, X } from 'lucide-react';
          ...
          {activeTab === 'gallery' && (
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {systemTemplates.map((template) => {
                ...
                const planLabel = template.accessPlan === 'STARTER' ? 'Free / Starter+' : `${template.accessPlan}+`;
                const activatedEntry = templates.find(
                  (t) => (t.templateCode === template.key || t.name === template.name) && t.templateType === 'SYSTEM_DEFAULT'
                );
                const isActivated = Boolean(activatedEntry);
                const isDefault = Boolean(activatedEntry?.default);
                return (
                  <article key={template.key} className="card-premium overflow-hidden flex flex-col">
                    <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden bg-gradient-to-br from-slate-50 to-brand-50/40 p-4">
                      ...
                      {isDefault && (
                        <div className="absolute top-2 left-2 flex items-center gap-1 rounded-full bg-emerald-500 px-2.5 py-1 shadow-sm">
                          <Star size={10} className="fill-white text-white" />
                          <span className="text-[10px] font-black text-white">Agency Default</span>
                        </div>
                      )}
                      {isActivated && !isDefault && (
                        <div className="absolute top-2 left-2 flex items-center gap-1 rounded-full bg-brand-500 px-2.5 py-1 shadow-sm">
                          <CheckCircle size={10} className="text-white" />
                          <span className="text-[10px] font-black text-white">Activated</span>
                        </div>
                      )}
                    </div>
                    <div className="flex flex-1 flex-col space-y-3 p-4">
                      ...
                      <div className="flex flex-wrap gap-1.5 text-[10px] font-bold">
                        <span className="rounded-lg bg-slate-100 px-2 py-1 text-slate-600">{template.language}</span>
                        <span className="rounded-lg bg-slate-100 px-2 py-1 text-slate-600">{template.pages}</span>
                        <span className="rounded-lg bg-slate-100 px-2 py-1 text-slate-600">Conditions: {template.hasConditions ? 'Yes' : 'No'}</span>
                        <span className={`rounded-lg px-2 py-1 ${locked ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>{planLabel}</span>
                      </div>
                      <div className="mt-auto grid grid-cols-2 gap-2">
                        {isActivated ? (
                          <>
                            <button
                              onClick={() => { setSelectedId(activatedEntry!.id); setActiveTab('preview'); }}
                              className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-200 transition-colors"
                            >
                              <Eye size={12} className="mr-1 inline" /> Open
                            </button>
                            {isDefault ? (
                              <span className="flex items-center justify-center gap-1 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  if (!activatedEntry) return;
                                  setSavingTemplate(true);
                                  try {
                                    const { data } = await api.post(`/contract-templates/${activatedEntry.id}/set-default`);
                                    const updated = unwrapData<Template>(data, data);
                                    setTemplates((current) => current.map((item) => ({ ...item, default: item.id === updated.id })));
                                    showToast(data?.message || 'Default template updated', 'success');
                                  } catch (err: any) {
                                    showToast(err?.response?.data?.message || err?.userMessage || 'Unable to set default template.', 'error');
                                  } finally {
                                    setSavingTemplate(false);
                                  }
                                }}
                                className="rounded-xl bg-brand-500 px-3 py-2 text-xs font-bold text-white disabled:opacity-50 hover:bg-brand-600 transition-colors"
                              >
                                Set Default
                              </button>
                            )}
                          ...
                          <>
                            <button onClick={() => setPreviewSystemTemplate(template)} className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-200 transition-colors">Preview</button>
                            <button
                              onClick={() => { showToast(`This template requires ${template.requiredPlan || template.accessPlan} plan.`, 'warning'); window.location.hash = '#/subscription'; }}
          ...
          <div className="w-full max-w-2xl rounded-3xl bg-white p-5 shadow-2xl">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-500">
                Agency scan template — upload your scanned paper below
              </div>
...
function DocumentThumbnail({ template, large = false }: { template: SystemTemplate; large?: boolean }) {
  const accentColor =
    template.accessPlan === 'PREMIUM' || template.accessPlan === 'ENTERPRISE' ? '#b45309' :
    template.accessPlan === 'STANDARD' ? '#7c3aed' :
    template.accessPlan === 'BASIC' ? '#2563eb' : '#059669';

  const isCompact = template.key === 'compact-one-page';
  const isLuxury = template.key === 'premium-luxury' || template.key === 'enterprise-custom';
  const showInspection = template.key?.includes('inspection') || isLuxury;
  const showConditions = Boolean(template.hasConditions);
  const isModern = template.key === 'modern-a4';

  const bgGradient = isLuxury
    ? 'from-amber-50 via-white to-amber-50/20'
    : isModern
    ? 'from-slate-50 to-white'
    : template.accessPlan === 'STANDARD'
    ? 'from-violet-50/20 to-white'
    : 'from-white to-slate-50/60';

  const docTitle = isLuxury ? 'Contrat Premium' : isCompact ? 'Contrat Rapide' : 'Contrat de Location';

  return (
    <div className={`${large ? 'mx-auto max-h-[560px] max-w-sm p-5' : 'w-44 p-3'} aspect-[210/297] overflow-hidden rounded-xl border border-slate-200 bg-gradient-to-br ${bgGradient} text-[6px] text-slate-700 shadow-lg`}>
      {/* Plan-tier accent stripe */}
      <div className="mb-2 -mx-3 -mt-3 h-1.5 rounded-t-xl" style={{ backgroundColor: accentColor }} />

      {/* Header */}
      <div className="flex items-start justify-between gap-2 border-b border-slate-200 pb-2">
        <div>
          <div className={`${large ? 'h-3 w-28' : 'h-2 w-20'} rounded`} style={{ backgroundColor: accentColor }} />
          <div className="mt-1 h-1.5 w-14 rounded bg-slate-300" />
          {!isCompact && <div className="mt-0.5 h-1 w-10 rounded bg-slate-200" />}
        </div>
        <div className={`${large ? 'h-9 w-9' : 'h-7 w-7'} flex shrink-0 items-center justify-center rounded border border-slate-200 bg-white text-[4px] font-bold text-slate-300 shadow-sm`}>
          LOGO
        </div>
      </div>

      {/* Title banner */}
      <div className="mt-2 rounded px-2 py-1 text-center font-black uppercase tracking-wider text-white" style={{ backgroundColor: accentColor }}>
        {docTitle}
      </div>

      {isCompact ? (
        <div className="mt-2 space-y-1.5">
          <DocSection title="Locataire" rows={3} accentColor={accentColor} />
          <DocSection title="Vehicule" rows={2} accentColor={accentColor} />
          <div className="grid grid-cols-2 gap-1">
            <MiniBox label="Depart" accentColor={accentColor} />
            <MiniBox label="Total" strong accentColor={accentColor} />
          </div>
        </div>
      ) : (
        <>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <DocSection title="Client" rows={isLuxury ? 5 : 4} accentColor={accentColor} />
            <DocSection title="Vehicule" rows={isLuxury ? 5 : 4} accentColor={accentColor} />
          </div>
          <div className="mt-2 grid grid-cols-3 gap-1.5">
            <MiniBox label="Depart" accentColor={accentColor} />
            <MiniBox label="Retour" accentColor={accentColor} />
            <MiniBox label="Total" strong accentColor={accentColor} />
          </div>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <DocSection title="Paiement" rows={3} accentColor={accentColor} />
            <div className="rounded border border-slate-200 bg-white p-1">
              <p className="mb-1 font-bold uppercase" style={{ color: accentColor, opacity: 0.75 }}>Documents</p>
              {['Carte grise', 'Assurance', 'Vignette', 'Permis'].map((item) => (
                <div key={item} className="mb-0.5 flex items-center gap-1">
                  <span className="h-1.5 w-1.5 rounded-sm border" style={{ borderColor: accentColor + '80' }} />
          ...
          {showInspection && (
            <div className="mt-2 rounded border bg-white p-1" style={{ borderColor: accentColor + '40' }}>
              <p className="font-bold uppercase" style={{ color: accentColor }}>Inspection</p>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">
        <SignatureBox label="Client" accentColor={accentColor} />
        <SignatureBox label="Agence" accentColor={accentColor} />
      </div>

      {showConditions && (
        <div className="mt-2 rounded border border-dashed bg-white/80 p-1" style={{ borderColor: accentColor + '60' }}>
          <p className="font-bold uppercase" style={{ color: accentColor }}>Conditions</p>
...
function DocSection({ title, rows, accentColor }: { title: string; rows: number; accentColor: string }) {
  return (
    <div className="rounded border border-slate-200 bg-white p-1">
      <p className="mb-1 font-bold uppercase" style={{ color: accentColor, opacity: 0.7 }}>{title}</p>
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="mb-0.5 h-1 rounded bg-slate-200" />
      ))}
    </div>
  );
}

function MiniBox({ label, strong = false, accentColor }: { label: string; strong?: boolean; accentColor: string }) {
  return (
    <div
      className="rounded border p-1"
      style={{
        borderColor: strong ? accentColor + '50' : '#e2e8f0',
        backgroundColor: strong ? accentColor + '12' : '#ffffff',
      }}
    >
      <p className="font-bold uppercase text-slate-400">{label}</p>
      <div className="mt-1 h-1 rounded" style={{ backgroundColor: strong ? accentColor + '60' : '#e2e8f0' }} />
    </div>
  );
}

function SignatureBox({ label, accentColor }: { label: string; accentColor: string }) {
  return (
    <div className="rounded border border-slate-200 bg-white p-1">
      <p className="font-bold uppercase text-slate-400">{label}</p>
      <div className="mt-2 h-6 rounded border border-dashed" style={{ borderColor: accentColor + '50' }} />
    </div>
  );
}
