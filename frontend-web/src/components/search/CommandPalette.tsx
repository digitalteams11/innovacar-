...
const sectionLabelKeys: Record<SearchResultType, string> = {
  ACTION: 'search.quickActions',
...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const quickActions = useMemo<GlobalSearchResult[]>(() => {
    ...
    if (hasPermission('CREATE_VEHICLE')) actions.push(action('action-add-vehicle', t('search.actionAddVehicle', 'Add vehicle'), t('search.actionAddVehicleDesc', 'Create a new fleet vehicle'), '/vehicles', 'car'));
    if (hasPermission('CREATE_RESERVATION')) actions.push(action('action-create-reservation', t('search.actionCreateReservation', 'Create reservation'), t('search.actionCreateReservationDesc', 'Open reservations to schedule a booking'), '/reservations', 'calendar'));
    if (hasPermission('CREATE_CONTRACT')) actions.push(action('action-create-contract', t('search.actionCreateContract', 'Create contract'), t('search.actionCreateContractDesc', 'Open contracts and start a rental contract'), '/contracts', 'file-text'));
    if (hasPermission('CREATE_CLIENT')) actions.push(action('action-add-client', t('search.actionAddClient', 'Add client'), t('search.actionAddClientDesc', 'Register a new customer profile'), '/clients', 'user'));
    if (hasPermission('EMPLOYEE_CREATE')) actions.push(action('action-add-employee', t('search.actionAddEmployee', 'Add employee'), t('search.actionAddEmployeeDesc', 'Invite an employee to this agency'), '/employees', 'users'));
    if (hasPermission('GPS_ACCESS')) actions.push(action('action-open-gps', t('search.actionOpenGps', 'Open GPS tracking'), t('search.actionOpenGpsDesc', 'Track connected vehicles'), '/gps-tracking', 'gps'));
    actions.push(action('action-open-billing', t('search.actionOpenBilling', 'Open billing'), t('search.actionOpenBillingDesc', 'Review plan, usage, and invoices'), '/settings?tab=billing', 'credit-card'));
    return actions;
  }, [hasPermission, t]);
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-start justify-center bg-black/45 px-2 py-3 backdrop-blur-sm sm:px-4 sm:py-[8vh]" role="dialog" aria-modal="true">
      <button className="absolute inset-0 cursor-default" onClick={onClose} aria-label={t('search.closeSearch', 'Close search')} />
      <section className="relative flex h-[calc(100vh-1.5rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-bg)] shadow-2xl backdrop-blur-2xl sm:h-auto sm:max-h-[78vh]">
        <header className="flex items-center gap-3 border-b border-[var(--border-subtle)] px-4 py-3">
          ...
          <input
            ...
            placeholder={t('search.searchPlaceholder', 'Search vehicles, clients, reservations, contracts...')}
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-3 sm:p-4">
          ...
          {!error && query.trim().length > 0 && query.trim().length < 2 && (
            <p className="px-2 text-sm text-[var(--text-muted)]">{t('search.typeAtLeast2Chars', 'Type at least 2 characters to search real data.')}</p>
          )}

          {!loading && !error && query.trim().length >= 2 && results.length === 0 && (
            <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] px-4 py-8 text-center">
              <p className="text-sm font-bold text-[var(--text-primary)]">{t('search.noMatchingRecords', 'No matching records found.')}</p>
              <p className="mt-1 text-xs text-[var(--text-muted)]">{t('search.tryPlateOrPhone', 'Try a plate number, phone, contract number, or client name.')}</p>
            </div>
          )}

          {!error && query.trim().length < 2 && recentResults.length > 0 && (
            <SearchSection title={t('search.recent', 'Recent')} results={recentResults} activeId={activeItem?.id} onSelect={selectResult} />
          )}

          {!error && Array.from(grouped.entries()).map(([type, items]) => (
            <SearchSection
              key={type}
              title={t(sectionLabelKeys[type]) || type}
              results={items}
          ...
          {!error && query.trim().length < 2 && visibleItems.length === 0 && (
            <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] px-4 py-8 text-center">
              <p className="text-sm font-bold text-[var(--text-primary)]">{t('search.startWithQuickAction', 'Start with a quick action or search your agency data.')}</p>
            </div>
          )}
        </div>
        ...
        <footer className="hidden items-center gap-3 border-t border-[var(--border-subtle)] px-4 py-2 text-[11px] text-[var(--text-muted)] sm:flex">
          ...
          <span>{t('search.enterKey', 'Enter')} {t('search.open', 'Open')}</span>
