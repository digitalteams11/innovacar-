...
export default function GpsConnectGuideModal({ isOpen, onClose, provider }: GpsConnectGuideModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('gps.connectGuideTitle', 'How to connect your GPS platform')} maxWidth="max-w-lg">
      <div className="space-y-5">
        ...
        {providerGuide ? (
          <div className="border-t border-[#e8e6e1]/60 dark:border-white/10 pt-4">
            <p className="text-sm font-semibold text-[#1e293b] dark:text-white mb-2">{t('gps.providerNotes', '{{provider}} notes', { provider: providerGuide.label })}</p>
          ...
          <div className="border-t border-[#e8e6e1]/60 dark:border-white/10 pt-4">
            <p className="text-sm text-slate-400">{t('gps.selectProviderHint', 'Select a GPS provider below to see provider-specific instructions here.')}</p>
          </div>
        )}
      </div>
    </Modal>
