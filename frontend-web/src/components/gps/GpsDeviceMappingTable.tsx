...
export default function GpsDeviceMappingTable({ devices, vehicles, onLinked }: GpsDeviceMappingTableProps) {
  ...
  const handleLink = async (device: GpsSyncDevice) => {
    ...
    if (!selectedId) {
      showToast(t('gps.selectVehicleToLink', 'Please select a vehicle to link this device.'), 'warning');
      return;
    }
    ...
    try {
      ...
      showToast(t('gps.deviceLinkedSuccess', 'Device linked to vehicle successfully.'), 'success');
    } catch {
      showToast(t('gps.deviceLinkFailed', 'Unable to link device to vehicle. Please try again later.'), 'error');
    } finally {
  ...
  if (devices.length === 0) {
    return (
      <div className="text-center py-8 text-sm text-slate-400">
        {t('gps.noDevicesFoundYet', 'No devices found yet. Click "Sync Devices" after a successful connection to load your provider\'s device list.')}
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto -mx-3 sm:mx-0">
      <table className="w-full text-sm min-w-[640px]">
        <thead>
          <tr className="text-left text-xs font-medium text-slate-400 border-b border-[#e8e6e1]/60">
            <th className="py-2 px-3">{t('gps.deviceImei')}</th>
            <th className="py-2 px-3">{t('gps.lastLocation')}</th>
            <th className="py-2 px-3">{t('gps.status')}</th>
            <th className="py-2 px-3">{t('gps.linkedVehicle')}</th>
            <th className="py-2 px-3">{t('gps.action')}</th>
          </tr>
        </thead>
            ...
            return (
              <tr key={device.deviceId}>
                ...
                <td className="py-2.5 px-3">
                  ...
                  >
                    <option value="">{t('gps.selectAVehicle', 'Select a vehicle…')}</option>
                    {vehicles.map((v) => (
                ...
                <td className="py-2.5 px-3">
                  ...
                  >
                    ...
                    {linked ? t('gps.change', 'Change') : t('gps.link', 'Link')}
                  </button>
                </td>
              </tr>
            );
      ...
      <div className="flex items-center gap-1.5 mt-3 px-3 sm:px-0 text-xs text-slate-400">
        <Car size={13} />
        <span>{t('gps.oneDevicePerVehicle')}</span>
