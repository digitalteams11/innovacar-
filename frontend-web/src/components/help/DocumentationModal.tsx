...
function buildPlainText(content: ModuleHelpContent): string {
  const lines = [`${content.title} ${i18n.t('help.guide', 'Guide')}`, '', content.documentation.overview, ''];
  ...
  if (content.documentation.requiredFields.length) {
    lines.push('', `${i18n.t('help.requiredFields', 'Required fields')}:`, ...content.documentation.requiredFields.map(field => `- ${field}`));
  }
  if (content.documentation.commonErrors.length) {
    lines.push('', `${i18n.t('help.commonErrors', 'Common errors')}:`, ...content.documentation.commonErrors.map(error => `- ${error}`));
  }
  if (content.documentation.tips.length) {
    lines.push('', `${i18n.t('help.tips', 'Tips')}:`, ...content.documentation.tips.map(tip => `- ${tip}`));
  }
  return lines.join('\n');
}
...
export default function DocumentationModal({ isOpen, onClose, content }: DocumentationModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${content.title} ${t('help.guide', 'Guide')}`} maxWidth="max-w-2xl">
      <div className="flex items-start justify-between gap-4">
        ...
        >
          ...
          {copied ? t('help.copied', 'Copied') : t('help.copy', 'Copy')}
        </button>
      </div>
      ...
      {content.documentation.keyActions.length > 0 && (
        <div className="mt-5">
          <h4 className="flex items-center gap-2 text-xs font-bold uppercase text-slate-400">
            <ListChecks size={14} /> {t('help.keyActions', 'Key actions')}
          </h4>
      ...
      {content.documentation.requiredFields.length > 0 && (
        <div className="mt-6">
          <h4 className="text-xs font-bold uppercase text-slate-400">{t('help.requiredFields', 'Required fields')}</h4>
          <div className="mt-2 flex flex-wrap gap-2">
      ...
      {content.documentation.commonErrors.length > 0 && (
        <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4">
          <h4 className="flex items-center gap-2 text-xs font-bold uppercase text-amber-700">
            <AlertTriangle size={14} /> {t('help.commonErrors', 'Common errors')}
          </h4>
      ...
      {content.documentation.tips.length > 0 && (
        <div className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-4">
          <h4 className="flex items-center gap-2 text-xs font-bold uppercase text-sky-700">
            <Lightbulb size={14} /> {t('help.tips', 'Tips')}
          </h4>
          <ul className="mt-2 space-y-1.5">
