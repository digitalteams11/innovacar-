...
export const GPS_FIELD_GUIDE: Record<string, GpsFieldGuideEntry> = {
  provider: {
    ...
    usage: 'Innovacar uses this provider to read vehicle location and sync devices automatically.',
  },
  appId: {
  ...
  apiKey: {
    title: 'API Key',
    text: 'This is the secret key that allows Innovacar to connect to your GPS platform on your behalf.',
    whereToFind: 'GPS provider dashboard → API Keys / Developer Settings → Generate API Key.',
    security: 'Never share this key publicly. Innovacar encrypts it before storing it and never displays it again in full — only a masked value is shown.',
  },
  baseUrl: {
    title: 'Base URL',
  ...
  deviceGroupId: {
    ...
    usage: 'If your provider does not use groups, leave this field empty — Innovacar will sync all available devices.',
  },
  webhookUrl: {
    title: 'Webhook URL',
    text: 'Optional. Some GPS providers can be configured to send location updates automatically instead of Innovacar pulling data on a schedule.',
    whereToFind: 'Paste the URL given by your GPS provider, or contact Innovacar support if your provider requires an inbound endpoint.',
    usage: 'If your provider does not support webhooks, leave this field empty — Innovacar will still sync via Test Connection / Sync Devices.',
  },
  testConnection: {
    title: 'Test Connection',
    text: 'After entering your credentials, click Test Connection. Innovacar will try to connect to your GPS provider and check whether devices can be loaded.',
  ...
  deviceMapping: {
    ...
    text: 'After a successful sync, Innovacar lists the GPS devices found on your provider account. Link each device to the correct vehicle in Innovacar so its live position appears correctly.',
    example: 'Device IMEI 864123456789123 → Vehicle Dacia Logan 123-A-45',
  },
  vehicleLinking: {
    title: 'Vehicle Linking',
    text: 'Each GPS device should be linked to exactly one vehicle. Linking is what allows Innovacar to show live location for that vehicle on the vehicle page, GPS map, and dashboard.',
  },
  lastSync: {
    title: 'Last Sync',
    text: 'Shows the last time Innovacar successfully pulled GPS data from your provider. If this is old or empty, click Sync Devices or Test Connection.',
  },
  activeDevices: {
  ...
  credentials: {
    ...
    text: 'Your API key is sensitive. Innovacar never displays it fully after it has been saved — only a masked indicator ("Stored securely") is shown. The key itself is encrypted at rest (AES-256) and is never logged.',
...
export const GPS_PROVIDER_GUIDE: Record<string, ProviderGuide> = {
  IOPGPS: {
    ...
    notes: [
      ...
      'Innovacar always connects to https://open.iopgps.com — this cannot be changed.',
  ...
  CUSTOM: {
    ...
    notes: [
      ...
      'You must enter both the Base URL and the API Key — Innovacar will call the configured endpoint to verify reachability.',
      'Device sync for custom providers may require additional integration work; contact Innovacar support for assistance.',
    ],
  },
};
