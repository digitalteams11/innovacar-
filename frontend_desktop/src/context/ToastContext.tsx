      ...
      <div className="fixed bottom-8 end-8 z-[160] flex w-[min(420px,calc(100vw-2rem))] flex-col gap-3" aria-live="polite">
        {toasts.map(toast => {
          ...
          return (
            <div
              key={toast.id}
              className={`flex items-start gap-3 border px-4 py-3 shadow-xl animate-in slide-in-from-right duration-300 ${styles}`}
              role={toast.type === 'error' ? 'alert' : 'status'}
            >
              <Icon size={20} className="mt-0.5 shrink-0" />
              <span className="flex-1 text-sm font-semibold leading-5">{toast.message}</span>
              <button onClick={() => dismiss(toast.id)} className="p-1 opacity-60 hover:opacity-100" aria-label="Dismiss notification">
                <X size={15} />
              </button>
            </div>
          );
