...
export default function Agency() {
  ...
  const saveAgency = async () => {
    ...
    try {
      ...
      setData(updated);
      showToast(t('agency.updateSuccess'), 'success');
    } catch (err) {
      showToast((err as any).userMessage || t('agency.updateFailed'), 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-5 animate-fade p-3 sm:p-4 lg:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 card-premium p-3 sm:p-5">
          <h3 className="text-base font-bold text-[#1e293b] mb-5">{t('agency.generalInformation')}</h3>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <label className="block text-sm font-medium text-[#1e293b] mb-2">{t('agency.email')}</label>
                <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                  className="w-full px-4 py-2.5 bg-[#f5f5f0] border border-[#e8e6e1] rounded-xl text-sm focus:outline-none focus:ring-2 ring-brand-100 focus:bg-white focus:border-brand-300 transition-all" />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#1e293b] mb-2">{t('agency.phone')}</label>
                <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                  className="w-full px-4 py-2.5 bg-[#f5f5f0] border border-[#e8e6e1] rounded-xl text-sm focus:outline-none focus:ring-2 ring-brand-100 focus:bg-white focus:border-brand-300 transition-all" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-[#1e293b] mb-2">{t('agency.address')}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[#1e293b] mb-2">{t('agency.taxId')}</label>
        ...
        <div className="space-y-4">
          <div className="card-premium p-3 sm:p-5">
            <h3 className="text-base font-bold text-[#1e293b] mb-4">{t('agency.contactInfo')}</h3>
