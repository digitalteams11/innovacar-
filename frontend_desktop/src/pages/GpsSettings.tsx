...
import {
  ...
  Trash2, Wifi, WifiOff, Activity, BookOpen
} from 'lucide-react';
...
export default function GpsSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast('Unable to save GPS information. Please try again later.', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSyncDevices = async () => {
    ...
    try {
      ...
      showToast('Unable to synchronize GPS devices. Please try again later.', 'error');
    } finally {
      setSyncing(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      showToast('Unable to remove GPS settings. Please try again later.', 'error');
    }
  };
