...
import { UserPlus, Mail, Lock, Car, Loader2, ArrowLeft, Building2, Eye, EyeOff, Check, X } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await register({ email, password, firstName, lastName, tenantId: tenantIdNum });
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2 ml-1" style={{ color: 'var(--text-primary)' }}>{t('register.tenantId', 'Tenant ID')}</label>
                <GlassInput icon={Building2} value={tenantId} onChange={setTenantId} placeholder="1" />
                <p className="text-xs mt-1 ml-1" style={{ color: 'var(--text-muted)' }}>{t('register.tenantHint', 'Ask your admin for the tenant ID')}</p>
              </div>
