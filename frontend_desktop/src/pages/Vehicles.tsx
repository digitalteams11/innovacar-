...
export default function Vehicles() {
  ...
  const saveVehicle = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.response?.data?.error || err.message || 'Failed to save vehicle';
      ...
      showToast(fullMsg);
      console.error('Vehicle save error:', err.response?.data);
    }
  };
  ...
  return (
    <div className="space-y-5 animate-fade">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-[#1e293b]">{t('vehicles.title')}</h1>
          <p className="text-slate-500 font-normal text-sm mt-0.5">{t('vehicles.subtitle')}</p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <button
            onClick={exportFleet}
            className="flex items-center justify-center gap-2 px-3 sm:px-5 py-2 sm:py-2.5 bg-white border border-[#e8e6e1] rounded-xl text-[#1e293b] font-medium text-xs sm:text-sm hover:bg-[#f5f5f0] active:scale-95 transition-all"
          >
            <Download size={18} />
            {t('vehicles.exportFleet')}
          </button>
          <button
            onClick={openCreate}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1 flex items-center gap-3 bg-white px-4 py-2.5 rounded-xl border border-[#e8e6e1] shadow-soft focus-within:ring-2 ring-brand-100/50 transition-all">
