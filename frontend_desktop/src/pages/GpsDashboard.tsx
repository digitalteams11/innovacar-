...
export default function GpsDashboard() {
  ...
  if (!settingsConfigured) {
    return (
      <div className="space-y-6 animate-fade max-w-6xl mx-auto w-full">
        <GlassPageHeader title="GPS Live Tracking" subtitle="Real-time fleet monitoring and analytics" icon={Satellite} />
        <div className="data-surface flex flex-col items-center justify-center py-16 text-center">
  ...
  return (
    <div className="space-y-4 animate-fade">
      {/* Header */}
      <GlassPageHeader title="GPS Live Tracking" subtitle="Real-time fleet monitoring and analytics" icon={Satellite} actions={
        <button
          onClick={fetchAllData}
          disabled={refreshing}
          className="surface-control flex items-center gap-2 h-10 px-4 text-sm font-medium transition-all disabled:opacity-50"
        >
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            <div key={stat.label} className="metric-surface min-h-[108px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4" style={{ minHeight: '520px', height: 'calc(100vh - 280px)' }}>
        {/* Map */}
        <div className="lg:col-span-3 relative rounded-lg overflow-hidden border border-[var(--border-subtle)] bg-[var(--bg-card)] shadow-[var(--shadow-card)]">
          <MapContainer
            center={mapCenter}
