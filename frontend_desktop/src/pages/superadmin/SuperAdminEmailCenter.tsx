...
export default function SuperAdminEmailCenter() {
  ...
  const handleDeleteTemplate = async (id: number) => {
    ...
    try {
      ...
      console.error(err);
      showToast(err?.response?.data?.message || 'Unable to delete template', 'error');
    }
  };
  ...
  const handleSendTest = async () => {
    try {
      ...
      setTestEmail('');
      await fetchData();
      showToast('Test email logged successfully', 'success');
    } catch (err: any) {
      console.error(err);
      showToast(err?.response?.data?.message || 'Unable to send test email', 'error');
    }
  };
