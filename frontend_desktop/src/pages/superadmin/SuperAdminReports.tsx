...
export default function SuperAdminReports() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const filename = res.data?.filename || `${selectedReport.id}-report.csv`;
      ...
      showToast(`Report generated with ${res.data?.rowCount || 0} rows`, 'success');
      setShowModal(false);
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="space-y-6 animate-fade">
      ...
      <div className="bg-white dark:bg-[#1a2332]/70 rounded-2xl p-4 sm:p-6 border border-[#e8e6e1]/80 dark:border-white/5 shadow-soft">
        ...
        <div className="rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-500 dark:bg-white/5">
          Scheduled report automation is not configured yet. Generated exports are available on demand.
        </div>
      </div>
