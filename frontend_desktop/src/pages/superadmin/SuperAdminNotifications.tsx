...
export default function SuperAdminNotifications() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error(err);
      showToast('Unable to load notifications. Please try again later.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const markAsRead = async (id: number) => {
    try {
      ...
      showToast('Unable to mark as read. Please try again later.', 'error');
    }
  };
  ...
  const dismissAll = async () => {
    ...
    try {
      ...
      showToast('Unable to mark all as read. Please try again later.', 'error');
    } finally {
      setDismissing(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {unreadCount > 0 && (
          <button
            onClick={dismissAll}
            disabled={dismissing}
            className="inline-flex items-center gap-2 bg-white dark:bg-[#1a2332]/70 hover:bg-slate-50 dark:hover:bg-white/5 text-[#1e293b] dark:text-white px-4 py-2.5 rounded-xl text-sm font-semibold border border-[#e8e6e1]/80 dark:border-white/5 shadow-soft transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <CheckCircle2 size={16} />
            {dismissing ? 'Saving...' : 'Mark All Read'}
          </button>
        )}
      </div>
