...
export default function AppearanceCustomizer() {
  ...
  const saveAppearance = async () => {
    ...
    try {
      ...
      showToast(data?.message || 'Appearance settings saved successfully.', 'success');
    } catch (err: any) {
      showToast((err as any).userMessage || 'Unable to save appearance settings. Please try again.', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-[var(--border-subtle)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-semibold text-[var(--text-primary)]">Appearance Studio</h3>
            <p className="text-xs text-[var(--text-muted)]">Changes preview instantly; click Save to persist them.</p>
          </div>
        </div>
      ...
      <section>
        <SettingLabel title="Display mode" description={`Currently using ${resolvedTheme} mode`} />
      ...
      <section>
        <SettingLabel title="Theme preset" description="Premium 2026 platform palettes — each one ships with a matching dark mode" />
      ...
      <section>
        <SettingLabel title="Brand colors" description="Tune the active preset to match your agency identity" />
        <div className="mt-3 grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <ColorControl label="Primary" value={appearance.primaryColor} onChange={(primaryColor) => updateAppearance({ primaryColor })} />
          <ColorControl label="Secondary" value={appearance.secondaryColor} onChange={(secondaryColor) => updateAppearance({ secondaryColor })} />
          <ColorControl label="Accent" value={appearance.accentColor} onChange={(accentColor) => updateAppearance({ accentColor })} />
          <ColorControl label="Sidebar" value={appearance.sidebarColor} onChange={(sidebarColor) => updateAppearance({ sidebarColor })} />
          <ColorControl label="Surface" value={appearance.surfaceColor} onChange={(surfaceColor) => updateAppearance({ surfaceColor, glassColor: surfaceColor })} />
        </div>
      </section>

      <section>
        <SettingLabel title="Glass rendering" description="Tune transparency, reflections, depth, and motion" icon={SlidersHorizontal} />
        <div className="mt-4 grid md:grid-cols-2 gap-x-8 gap-y-5">
          <RangeControl label="Glass intensity" value={appearance.glassIntensity} unit="%" onChange={(glassIntensity) => updateAppearance({ glassIntensity })} />
          <RangeControl label="Opacity" value={appearance.opacity} unit="%" onChange={(opacity) => updateAppearance({ opacity })} />
          <RangeControl label="Blur" value={appearance.blur} min={0} max={40} unit="px" onChange={(blur) => updateAppearance({ blur })} />
          <RangeControl label="Glass depth" value={appearance.depth} unit="%" onChange={(depth) => updateAppearance({ depth })} />
          <RangeControl label="Shadow strength" value={appearance.shadowStrength} unit="%" onChange={(shadowStrength) => updateAppearance({ shadowStrength })} />
          <RangeControl label="Animation speed" value={appearance.animationSpeed} min={40} max={180} unit="%" onChange={(animationSpeed) => updateAppearance({ animationSpeed })} />
          <RangeControl label="Corner radius" value={appearance.cornerRadius} min={0} max={16} unit="px" onChange={(cornerRadius) => updateAppearance({ cornerRadius })} />
        </div>
      </section>
      ...
      <section className="grid md:grid-cols-3 gap-4">
        ...
        <label className="surface-control min-h-16 px-3 flex items-center justify-between md:col-span-2">
          <span>
            <span className="block text-xs font-semibold text-[var(--text-primary)]">White label mode</span>
            <span className="block text-[10px] text-[var(--text-muted)]">Use agency identity throughout the shell</span>
          </span>
          <input
            type="checkbox"
      ...
      <section>
        <SettingLabel title="Live preview" description="What the shell looks like with these settings applied" icon={Layers} />
        <LivePreview
      ...
      <div className="flex-1 p-3 space-y-2.5">
        ...
        <div className="rounded-lg p-3 space-y-2" style={{ background: surfaceColor, border: '1px solid rgba(0,0,0,0.06)' }}>
          <div className="flex items-center justify-between">
            ...
            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ background: accentColor }}>NEW</span>
          </div>
          <span className="block h-2 w-32 rounded-full bg-black/[0.06]" />
          <div className="flex items-center gap-2 pt-1">
            <span className="h-6 px-3 rounded-lg text-[9px] font-bold text-white flex items-center" style={{ background: primaryColor }}>Primary action</span>
            <span className="h-6 px-2 rounded-lg text-[9px] flex items-center border border-black/10 bg-white/40">Input</span>
          </div>
        </div>
      </div>
    </div>
