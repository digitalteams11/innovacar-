...
const eventLabels: { event: SoundEvent; label: string; description: string }[] = [
  { event: 'newReservation', label: 'New Reservation', description: 'Soft booking chime' },
  { event: 'contractSigned', label: 'Contract Signed', description: 'Clean confirmation tone' },
  { event: 'paymentReceived', label: 'Payment Received', description: 'Premium success tone' },
  { event: 'vehicleReturned', label: 'Vehicle Returned', description: 'Soft completion tone' },
  { event: 'gpsAlert', label: 'GPS Alert', description: 'Short warning pulse' },
  { event: 'subscriptionExpiring', label: 'Subscription Expiring', description: 'Subtle warning tone' },
  { event: 'error', label: 'Error / Failed Action', description: 'Low soft error tone' },
  { event: 'supportMessage', label: 'New Support Message', description: 'Light message pop' },
];
...
export default function SoundSettings() {
  ...
  const saveSoundSettings = async () => {
    ...
    try {
      ...
      showToast(data?.message || 'Sound settings saved successfully', 'success');
    } catch (err: any) {
      showToast((err as any).userMessage || 'Unable to save sound settings. Please try again.', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <section className="space-y-4 rounded-2xl border border-slate-100 bg-white p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-bold text-[#1e293b]">Sounds</h4>
          </div>
          <p className="mt-1 text-xs text-slate-500">Premium, subtle audio feedback for important agency events.</p>
        </div>
      ...
      {!audioReady && (
        <div className="rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-700">
          Browser audio unlocks after the first click or key press. Visual notifications always remain active.
        </div>
      )}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <ToggleCard
          ...
          description="Turn all notification sounds on or off."
        ...
        <ToggleCard
          ...
          description="Mute all sounds until disabled."
      ...
      <div className="space-y-2">
        <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Volume</label>
        <input
          type="range"
          min={0}
          max={1}
      ...
      <div className="space-y-2">
        <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Sound theme</label>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {(['minimal', 'premium', 'automotive', 'silent'] as const).map((theme) => (
            <button key={theme} type="button" onClick={() => updatePreferences({ theme })}
              ...
              {theme === 'silent' ? 'Silent Mode' : theme}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <ToggleCard title="Mute during night mode" description="Quiet hours: 22:00 to 07:00." checked={preferences.muteNightMode}
          onChange={(checked) => updatePreferences({ muteNightMode: checked })} />
        <ToggleCard title="Mute system alerts" description="Suppress GPS, subscription, and error sounds." checked={preferences.muteSystemAlerts}
          onChange={(checked) => updatePreferences({ muteSystemAlerts: checked })} />
        <ToggleCard title="Important events only" description="Play sound only for priority events." checked={preferences.importantOnly}
      ...
      <div className="flex flex-wrap gap-2">
        {[15, 60, 240].map((minutes) => (
          <button key={minutes} type="button" onClick={() => temporarilyMute(minutes)}
            ...
            Mute {minutes < 60 ? `${minutes} min` : `${minutes / 60}h`}
          </button>
        ))}
      </div>

      <div className="space-y-2">
        <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Events</label>
