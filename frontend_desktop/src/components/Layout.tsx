...
import {
  BarChart3, Building2, Calendar, Car, ChevronDown, CreditCard, Crown, FileText, HelpCircle,
  Info, LayoutDashboard, LockKeyhole, LogOut, MapPin, MoreHorizontal,
  Palette, PanelLeftClose, PanelLeftOpen, Search, Settings, ShieldCheck,
  Users, Wrench, X,
} from 'lucide-react';
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // /subscription and /settings (see App.tsx ProtectedRoute) — the sidebar
    ...
    <div className="app-shell min-h-screen flex">
      <aside className={cn(
        ...
        <button
          onClick={logout}
          className={cn(
            'm-3 mt-0 min-h-10 flex items-center justify-center gap-2 rounded-lg text-sm text-rose-300/75 hover:bg-rose-500/10 hover:text-rose-200',
            (appearance.sidebarStyle === 'compact' || sidebarCollapsed) && 'mx-auto w-10',
          )}
          title={appearance.sidebarStyle === 'compact' || sidebarCollapsed ? t('layout.logout') : undefined}
        >
          <LogOut size={17} />
          {appearance.sidebarStyle !== 'compact' && !sidebarCollapsed && t('layout.logout')}
        </button>
      </aside>
      ...
      <div className={cn(
        ...
        <header className="app-topbar sticky top-2 lg:top-4 z-40 h-[58px] lg:h-[64px] mx-2 lg:mx-4 px-3 sm:px-4 lg:px-5 flex items-center gap-3 rounded-lg">
          ...
          <label className="surface-control hidden md:flex h-9 w-full max-w-[420px] items-center gap-2 px-3">
            ...
            <input className="w-full bg-transparent outline-none text-sm placeholder:text-[var(--text-muted)]" placeholder={t('layout.searchPlaceholder')} />
            <kbd className="text-[9px] text-[var(--text-muted)] border border-[var(--border-subtle)] px-1.5 py-0.5 rounded">Ctrl K</kbd>
          </label>
